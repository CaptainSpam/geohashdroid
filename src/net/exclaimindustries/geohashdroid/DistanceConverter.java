...
public class DistanceConverter {
	...
	/**
	 * Do the actual conversion.  This will attempt to get whatever preference
	 * is set for the job and, using the given DecimalFormat, convert it into
	 * a string, suitable for displaying.
	 * 
	 * @param c the context from which to get the preferences
	 * @param df the format of the string
	 * @param distance the distance, as returned by Location's distanceTo method
	 * @return a String of the distance, with units marked
	 */
	public static String makeDistanceString(Context c, DecimalFormat df, float distance) {
		// First, get the current unit preference.
		SharedPreferences prefs = c.getSharedPreferences(GeohashDroid.PREFS_BASE, 0);
		String units = prefs.getString(c.getResources().getString(R.string.pref_units_key), "Metric");
