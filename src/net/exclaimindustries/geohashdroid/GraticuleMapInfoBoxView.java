...
public class GraticuleMapInfoBoxView extends TextView {
	...
	public GraticuleMapInfoBoxView(Context context, AttributeSet attrs, int defStyle) {
	 ...
	 * Location is needed.
	 * 
	 * @param grat Currently selected Graticule
	...
	public void update(Graticule grat, Location loc) {
		...
		if(grat == null) {
			...
			setText(getContext().getString(R.string.graticule_label) + " "
				+ grat.getLatitude() + (grat.isSouth() ? 'S' : 'N') + " "
				+ grat.getLongitude() + (grat.isWest() ? 'W' : 'E')
