...
/**
 * Retrieves and constructs the hash for a given day.  By default, this pulls
 * in the current date's hash (or the most recent date if today can't be
 * retrieved for whatever reason).  It can also be told to find any past date's
 * hash.
 * 
 * This implementation uses the peeron.com site to get the DJIA (http://irc.peeron.com/xkcd/map/data/2008/12/03).
 * 
...
public class HashMaker implements Serializable {
	...
	/**
	 * Creates a new HashMaker using the current date.  This goes to the web
	 * for stock data right away.
	 * 
	 * @param graticule initial Graticule to use for this
	 * @throws FileNotFoundException the date could not be found on the server (usually means the data isn't up yet)
	 * @throws IOException some other sort of IOException occurred
	 */
	public HashMaker(Graticule graticule) throws FileNotFoundException, IOException {
		// Grab today from Calendar.
		this(graticule, Calendar.getInstance());
	}
	
	/**
	 * Creates a new HashMaker using the given Calendar object.  This goes to
	 * the web for stock data right away.
	 * 
	 * @param graticule initial Graticule to use for this
	 * @param cal Calendar from which the date shall spring
	 * @throws FileNotFoundException the date could not be found on the server (usually means the data isn't up yet)
	 * @throws IOException some other sort of IOException occurred
	 */
	public HashMaker(Graticule graticule, Calendar cal) throws FileNotFoundException, IOException {
		this.graticule = graticule;
		setCalendar(cal);
	}
	
	/**
	 * Creates a new HashMaker using the given date data.  Months are from
	 *  0 to 11, like in a Calendar object.  Or, more appropriately, a
	 *  DatePicker object.  Aaaaand this goes to the web for stock data right
	 *  away.
	 * 
	 * @param graticule initial Graticule to use for this
	 * @param year four-digit year
	 * @param month current month, 0 to 11 (NOT 1 to 12)
	 * @param day current day of the month
	 * @throws FileNotFoundException the date could not be found on the server (usually means the data isn't up yet)
	 * @throws IOException some other sort of IOException occurred
	 */

	public HashMaker(Graticule graticule, int year, int month, int day) throws FileNotFoundException, IOException {
		this.graticule = graticule;
	...
	private synchronized void fetchStock() throws FileNotFoundException, IOException {
		mAborted = false;
		String sMonthStr;
		String sDayStr;
		...
		String location = "http://irc.peeron.com/xkcd/map/data/" + sCal.get(Calendar.YEAR) + "/" + sMonthStr + "/" + sDayStr;
		...
		try {
			...
			throw new IOException("The stock server was contacted, but it wasn't returning parseable stock data.");
		}
	...
	private void makeHash() {
		...
		String fullLine = cal.get(Calendar.YEAR) + "-" + monthStr + "-" + dayStr + "-" + stock;
	...
	/**
	 * Gets the current effective stock date, accounting for the 30W Rule.
	 * This does NOT return the nearest Friday on a weekend, though.
	 * 
	...
	/**
	 * Sets a new Calendar to use as this date.  This will also reset the
	 * current stock calendar based on the 30W Rule AND refetch the stock.
	 * 
	 * @param cal new Calendar for a date
	 * @throws FileNotFoundException the date could not be found on the server (usually means the data isn't up yet)
	 * @throws IOException some other sort of IOException occurred
	 */
	public synchronized void setCalendar(Calendar cal) throws FileNotFoundException, IOException {
		this.cal = cal;
	 ...
	 * 
	 * @param graticule the new Graticule
	 */
	public synchronized void setGraticule(Graticule graticule) throws FileNotFoundException, IOException {
	 ...
	 * called, the caller MUST NOT read any hash or stock data from this
	 * object, as it's unreliable.
	 */
	public void abort() {
		if(mRequest != null) {
			mRequest.abort();
     ...
     */
    protected static String getStringFromStream(InputStream stream) throws IOException {
     ...
     * HashMaker.  That's passed to everything as need be.  Which makes a lot
     * more sense than passing a HashMaker all over the place, since HashMaker
     * should just be a worker object, not a data storage object.  I think.
     * 
     * @return an Info object with everything that the rest of Geohash Droid will need.
     */
    public Info makeInfo() {
    	return new Info(getLatitude(), getLongitude(), getGraticule(), getCalendar());
    }
}
