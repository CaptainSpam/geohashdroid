...
public final class HumanBytes {
    ...
    public static String toIdeal(long input) {
        ...
        // First off, if this is less than 1024 (by absoluteness), just return it.
        if(Math.abs(input) < 1024) return input + "B";
        
        // Now, go in order from biggest to smallest.  Comparisons are easier
        // on the system than comparisons and divisions.
        if(Math.abs(input) >= ONEPETA) return toPeta(input);
        if(Math.abs(input) >= ONETERA) return toTera(input);
        if(Math.abs(input) >= ONEGIG) return toGiga(input);
        if(Math.abs(input) >= ONEMEG) return toMega(input);
        if(Math.abs(input) >= ONEKILO) return toKilo(input);
    ...
    /**
     *<p>
     * Finishes up output from a conversion.  This means putting the right decimals
     * in, adding commas, and attaching the suffix.
     *</p>
     *
     * @param input number to convert
     * @param suffix suffix to attach to the result
